...
```{tikz, figure_1, fig.cap = "Nodes anchored on a curved line", fig.align = "center"}
...
```{tikz, figure_2, fig.cap = "Multi-segment arrow example", fig.align = "center"}
% Loading dependencies
\usetikzlibrary{
  positioning, 
  fit, calc,
...
Finally, this experience was not only usefull for learning some new *tikz* trics, but for working with tables as well. 
For example, I believe everyone knows that it's possible to embedd `itemized` lists inside `tabularx` environment. 
However, it is possible to do the same thing with ususal `tabular` environment as well!
